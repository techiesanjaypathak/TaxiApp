...
extension UIView {
    ...
    func inputContainer(withTextField textField:UITextField? = nil, withSegmentedControl segmentedControl: UISegmentedControl? = nil, image: UIImage) -> UIView {
        let view = UIView()
        ...
        if let segmentedControl = segmentedControl {
            ...
            segmentedControl.anchor(top:imageView.bottomAnchor, left: view.leftAnchor, right: view.rightAnchor, paddingLeft: 8, paddingRight: 8, height: 40)
...
extension UIButton {
    func customAttributedNavigationButton(withTitle title: String, message msg: String, _ target: Any?, action: Selector) -> UIButton {
        let button = UIButton()
        let attributedTitle = NSMutableAttributedString(string: title,
                                                        attributes: [NSAttributedString.Key.font: UIFont.boldSystemFont(ofSize: 16),
                                                                    NSAttributedString.Key.foregroundColor: UIColor.systemGray2])
        attributedTitle.append(NSAttributedString(string: msg, attributes: [NSAttributedString.Key.font : UIFont.boldSystemFont(ofSize: 16), NSAttributedString.Key.foregroundColor: UIColor.customButtonColor]))
...
extension MKPlacemark {
    var address: String? {
        get {
            ...
            return "\(localSubThoroughfare) \(localStreetName), \(localLocality), \(localSubLocality), \(localAdministrativeArea), \(localSubAdministrativeArea)"
        }
    }
}
